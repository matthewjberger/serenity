...
impl View {
    ...
    pub fn render<'rp>(
        ...
        if self.vertex_buffer.size() as usize
            != world.vertices.len() * std::mem::size_of::<crate::scene::Vertex>()
        {
            log::info!("Resizing geometry buffers...");
        ...
        if let Some(active_scene) = world.active_scene_index {
            let scene = &world.scenes[active_scene];
            scene.walk_dfs(|_, node_index| {
                mesh_ubos[ubo_offset] = DynamicUniform {
                    model: scene.global_transform(node_index, &world.transforms, &world.nodes),
                };
                ubo_offset += 1;
            });
        }
        ...
        if let Some(active_scene) = world.active_scene_index {
            ...
            scene.walk_dfs(|node_index, _graph_node_index| {
                ubo_offset += 1;
                if let Some(mesh) = world.meshes.get(node_index) {
...
pub fn create_camera_matrices(
    ...
    let scene = match world.active_scene_index {
        Some(active_scene_index) => &world.scenes[active_scene_index],
        None => return result,
    };
    scene.walk_dfs(|node_index, _graph_node_index| {
        let camera = match scene.active_camera_index {
            Some(active_camera_index) => &world.cameras[active_camera_index],
            None => return,
        };
        let transform = match world.nodes[node_index].transform_index {
            Some(transform_index) => &world.transforms[transform_index],
            None => return,
        };
        result = Some((
