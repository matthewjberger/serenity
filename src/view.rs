use crate::world::{
    create_camera_matrices, AlphaMode, Image, ImageFormat, MagFilter, MinFilter, PrimitiveTopology,
...
impl WorldRender {
    pub fn new(gpu: &crate::gpu::Gpu, world: &World) -> Self {
        ...
        if texture_views.is_empty() {
            texture_views.push(textures[0].create_view(&wgpu::TextureViewDescriptor::default()));
        }
    ...
    pub fn render<'rp>(
        ...
        let Some(scene) = world.scenes.first() else {
            return;
        };

        let (camera_position, projection, view) =
            create_camera_matrices(world, scene, gpu.aspect_ratio());
        ...
        scene
            ...
            .for_each(|(index, graph_node_index)| {
                mesh_ubos[index] = DynamicUniform {
                    ...
                    number_of_instances: world.nodes[scene.graph[graph_node_index]]
                        .instance_indices
                        .len() as _,
                    ..Default::default()
                };
                ...
                .for_each(|(ubo_index, graph_node_index)| {
                    ...
                    if let Some(mesh_index) = node.mesh_index {
                        ...
                        for primitive in mesh.primitives.iter() {
                            ...
                            let instance_range = if node.instance_indices.is_empty() {
                                0..1
                            } else {
                                0..(node.instance_indices.len() as u32)
                            };
...
fn create_texture(gpu: &crate::gpu::Gpu, image: &Image) -> wgpu::Texture {
...
fn vertex_main(vert: VertexInput, instance: InstanceInput) -> VertexOutput {
    ...
    if mesh_ubo.number_of_instances > 0u  {
