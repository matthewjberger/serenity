...
impl QuadPipeline {
    pub fn new(gpu: &crate::render::gpu::Gpu, world: &crate::world::World) -> Self {
        assert!(world.fonts.len() > 0, "No fonts are loaded!");
        // Texture the char quads using this font atlas with the glyph offsets
        let font_atlas = world.font_atlas(0);

        let mut instances = vec![];
        let mut x_offset = 0.0;
        let mut y_offset = 0.0;
        let font_positions = world.font_positions(0, "Hello, world!");
        for position in font_positions {
            ...
            let x = x_offset + x as f32;
            let y = y_offset + y as f32;
            instances.push(dbg!(Instance {
                position: nalgebra_glm::Vec3::new(x, y, 0.0),
...
const VERTICES: [Vertex; 4] = [
    Vertex {
        position: nalgebra_glm::Vec3::new(1.0, -1.0, 0.0),
    },
    Vertex {
        position: nalgebra_glm::Vec3::new(-1.0, -1.0, 0.0),
    },
